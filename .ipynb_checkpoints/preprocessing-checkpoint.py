...
def preprocessing(img_dir, img_fn, crop_idx_dim1=1300, thresh_refl=0.15, area_range=1000, verbose=True):
    """
    Take an image and create sub-images for each grain

    :param img_dir: PATH of hyperspectral images
    :param img_fn: name of a hyperspectral image
    ...
    """
    img_init = sp.open_image(img_dir + img_fn + '.hdr')
    img = img_init
    ...
    try:
        df = pd.read_csv(img_dir + "/csv/brightest_bands.csv", index_col=0)
    except:
        retrieve_all_brightest_bands_to_csv(img_dir)
        df = pd.read_csv(img_dir + "/csv/brightest_bands.csv", index_col=0)
    band, max_ref = df.loc[img_fn]
    band = int(band)
